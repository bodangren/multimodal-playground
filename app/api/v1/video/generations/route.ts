...
import { executeVideoGeneration, OpenAIVideoRequestSchema } from '@/lib/openai-compat/video';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const parsed = OpenAIVideoRequestSchema.parse(body);
    const result = await executeVideoGeneration(parsed);
