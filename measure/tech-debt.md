...
| 2026-04-23 | provider_failover_20260423 | `FallbackChain.execute()` infinite loop when all providers have open circuits — the `attempts` counter never increments for skipped providers. | High | Resolved | Added cycle detection with `triedInCycle` Set; now properly aborts with "All providers have open circuits" error. |
...
| 2026-04-24 | advanced_provider_routing_20260424 | `executor.ts` passes async `recordSuccess`/`recordFailure` callbacks to `ProviderFallbackConfig` which expects sync `() => void`. The returned Promises are fire-and-forget — circuit breaker state updates may lag and errors are silently swallowed. | Medium | Open | Make `ProviderFallbackConfig.recordSuccess`/`recordFailure` async-aware or await them in `FallbackChain.execute()`. |
